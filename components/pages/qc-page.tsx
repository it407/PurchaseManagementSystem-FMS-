    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        for (let i = 6; i < result.data.length; i++) {
          ...
          if (planned5 && planned5.trim() !== "" && (!actual5 || actual5.trim() === "")) {
            pendingRecords.push({
              id: `row-${i + 1}`,
          ...
          else if (planned5 && planned5.trim() !== "" && actual5 && actual5.trim() !== "") {
            historyRecords.push({
              id: `row-${i + 1}`,
              indentNumber: row[1] || "", // Column B: Indent Number
              productNo: row[2] || "", // Column C: Product No
              ...
              approvalStatus: row[38] || "Approved", // Column AK
              status: "QC Done",
              rowIndex: i + 1,
            });
          }
...
const openCancelModal = (record: Record) => {
  ...
  return (
    <div className="space-y-6 p-4 md:p-0">
      {/* Header */}
      <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Quality Check (QC)</h2>
    ...
    Cancel QC Check
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {history.map((record) => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900">{record.indentNumber}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{record.productNo}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm font-medium text-gray-900">{record.qcReportNo}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{record.poNo}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-900">{record.materialName}</td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${record.sampleResult === "Pass"
                              ? "bg-green-100 text-green-800"
                              : "bg-red-100 text-red-800"
                              }`}
                          >
                            {record.sampleResult === "Pass" ? <CheckCircle className="w-3.5 h-3.5 mr-1" /> : <XCircle className="w-3.5 h-3.5 mr-1" />}
                            {record.sampleResult}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${record.approvalStatus === "Approved"
                ...
                {history.map((record) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span
                        className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${record.sampleResult === "Pass"
                          ? "bg-green-100 text-green-800"
                          : "bg-red-100 text-red-800"
                          }`}
                      >
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <LabeledInput label="Material Name" value={selectedRecord?.materialName || ""} onChange={() => { }} disabled />

          <LabeledInput
            label="QC Report No."
            value={formData.qcReportNo}
            onChange={(e) => setFormData({ ...formData, qcReportNo: e.target.value })}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, sampleResult: e.target.value })}
  ...
  <form onSubmit={handleCancel} className="space-y-4">
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
      <p className="text-sm text-yellow-800">
        Are you sure you want to cancel this quality check record? This action cannot be undone.
      </p>
    </div>
